...
const char *CMDS[] = { "penup", "pendown", "colour", "forward", "backward", "right", "left", "loop" };
...
void printSingleCommand(CmdNode *p, int counter, char *indent) {
  if ((0 == strcmp(p->cmd, "penup")) ||
      (0 == strcmp(p->cmd, "pendown")) ||
      (0 == strcmp(p->cmd, "right")) ||
      (0 == strcmp(p->cmd, "left"))) {
    printf("%3d: %s%s\n", counter, indent, p->cmd);
  } else {
    printf("%3d: %s%s %d\n", counter, indent, p->cmd, p->val);
  }
}
...
void printCommandList(CmdNode *head) {
  ...
  while (p != NULL) {
    if ((0 == strcmp(p->cmd, "loop"))) {
      printSingleCommand(p, counter, "");
      counter++;
      loop = p->loop_head;
      while (loop != NULL) {
        printSingleCommand(loop, counter, "  ");
        loop = loop->next;
        counter++;
      }
    } else {
...
void insertCommandLoop(CmdNode *node, CmdNode *new_CmdNode) {
  CmdNode *loop_head = node->loop_head;
  CmdNode *tail = NULL;

  if (node->loop_head == NULL) {
    node->loop_head = new_CmdNode;
    return;
  } else {
    tail = node->loop_head;
    while (tail->next != NULL) {
...
void runProcess(Image *im, CmdNode *head, int startX, int startY, int *curColor, int *curDir, int *penDown, int *endX, int *endY) {
  ...
  while (p != NULL) {
    if (0 == strcmp(p->cmd, "loop")) {
      for (int i=0; i<p->val; i++) {
        runProcess(im, p->loop_head, curX, curY, curColor, curDir, penDown, &nextX, &nextY);
        curX = nextX;
        curY = nextY;
      }
    } else if (0 == strcmp(p->cmd, "penup")) {
      *penDown = 0;
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
  ...
  runProcess(im,       /* Image */ 
            ...
            0,         /* StartY */   
            &curColor, /* Color */   
            &curDir,   /* Direction:  0:UP   1:RIGHT   2:DOWN   3:LEFT */
            &penDown,  /* PenDown:    0:UP   1:DOWN */
            endX,      /* endX */
            endY);     /* endY */ 
